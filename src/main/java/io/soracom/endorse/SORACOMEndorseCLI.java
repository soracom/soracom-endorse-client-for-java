...
public class SORACOMEndorseCLI {
	...
	public static class EndorseCLIOptions {
		...
		public static final Option keysEndpointUrlOption = Option.builder().longOpt("keys-api-endpoint-url")
				.hasArg(true)
				.desc("Override the default Keys API endpoint url with this switch.\n(eg --keys-api-endpoint-url=https://g.api.soracom.io)")
				.build();
		public static final Option interfaceOption = Option.builder().longOpt("interface").hasArg(true).desc(
				"UICC Interface to use. Valid values are [iso7816, comm, mmcli, autoDetect]. 'autoDetect' is used as default.")
				.build();
		public static final Option portNameOption = Option.builder().longOpt("port-name").hasArg(true)
				.desc("Port name of communication device.(eg --port-name COM1 or --port-name /dev/tty1)").build();
		public static final Option baudRateOption = Option.builder().longOpt("baud-rate").hasArg(true)
				.desc("Baud rate for communication device.(eg --baud-rate 57600)").build();
		public static final Option dataBitOption = Option.builder().longOpt("data-bit").hasArg(true)
				.desc("Data bits for communication device.(eg --data-bit 8)").build();
		public static final Option stopBitOption = Option.builder().longOpt("stop-bit").hasArg(true)
		...
		public static final Option deviceInfoOption = Option.builder().longOpt("deviceInfo")
				.desc("Query the Communication device and print the information").build();

		public static final Option disableKeyCacheOption = Option.builder().longOpt("disableKeyCache").desc(
				"Disable key cache.\nIf you want to set a encryption key of the keystore, please set a value as environment variable.\n"
						+ KeyCache.ENV_NAME_ENDORSE_KEY_STORE_KEY + "=xxx")
				.build();
		public static final Option clearKeyCacheOption = Option.builder().longOpt("clearKeyCache")
				.desc("Clear key cache").build();
	...
	private static void displayHelp(Options options) {
		...
		helpText.append("\r\n");
		helpText.append("To execute SIM(AKA) authentication using interface auto-detection: \r\n");
		helpText.append("soracom-endorse \r\n");
		helpText.append("\r\n");
		helpText.append("To execute SIM(AKA) authentication using a card reader: \r\n");
		helpText.append("soracom-endorse --interface iso7816 \r\n");
		helpText.append("\r\n");
	...
	public static SORACOMEndorseClientConfig createSORACOMEndorseClientConfig(CommandLine commandLine) {
		...
		if (commandLine.hasOption(EndorseCLIOptions.keysEndpointUrlOption.getLongOpt())) {
			clientConfig.setApiEndpointUrl(
					commandLine.getOptionValue(EndorseCLIOptions.keysEndpointUrlOption.getLongOpt()));
		}
		if (commandLine.hasOption(EndorseCLIOptions.interfaceOption.getLongOpt())) {
			clientConfig.setUiccInterfaceType(UiccInterfaceType
					.valueOf(commandLine.getOptionValue(EndorseCLIOptions.interfaceOption.getLongOpt())));
		}
	...
	public static CommunicationDeviceConfig createCommunicationDeviceConfig(CommandLine commandLine) {
		...
		if (commandLine.hasOption(EndorseCLIOptions.portNameOption.getLongOpt())) {
			communicationDeviceConfig
					.setPortName(commandLine.getOptionValue(EndorseCLIOptions.portNameOption.getLongOpt()));
		}
		if (commandLine.hasOption(EndorseCLIOptions.baudRateOption.getLongOpt())) {
			communicationDeviceConfig.setBaudRate(
					Integer.parseInt(commandLine.getOptionValue(EndorseCLIOptions.baudRateOption.getLongOpt())));
		}
		if (commandLine.hasOption(EndorseCLIOptions.dataBitOption.getLongOpt())) {
			communicationDeviceConfig.setDataBits(
					Integer.parseInt(commandLine.getOptionValue(EndorseCLIOptions.dataBitOption.getLongOpt())));
		}
		if (commandLine.hasOption(EndorseCLIOptions.stopBitOption.getLongOpt())) {
			communicationDeviceConfig.setStopBits(
					Integer.parseInt(commandLine.getOptionValue(EndorseCLIOptions.stopBitOption.getLongOpt())));
		}
		if (commandLine.hasOption(EndorseCLIOptions.parityBitOption.getLongOpt())) {
			communicationDeviceConfig.setParity(
					Integer.parseInt(commandLine.getOptionValue(EndorseCLIOptions.parityBitOption.getLongOpt())));
		}
		if (commandLine.hasOption(EndorseCLIOptions.modemManagerIndexOption.getLongOpt())) {
			int modemIntex = Integer
					.parseInt(commandLine.getOptionValue(EndorseCLIOptions.modemManagerIndexOption.getLongOpt()));
